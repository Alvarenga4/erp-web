...
function ButcheryCard({ data }) {
  ...
  const selectedButchery = useSelector((state) => state.data);
  ...
  const searchFilterFunction = (text) => {
    ...
    if (text) {
      const newData = data.filter(function (item) {
        const itemData = item.cnpj ? item.cnpj.toUpperCase() : "".toUpperCase();
  ...
  const handleForm = async (id) => {
    try {
      dispatch({ type: "ADD_BUTCHERY_ID", id: id });
      console.log('Anterior: ', selectedButchery)
    } catch (err) {
      console.log(err);
    }
  };
  ...
  return (
    <div>
      <GridContainer>
        <GridItem
          xs={12}
          sm={status === "Approved" ? 4 : 8}
          md={status === "Approved" ? 4 : 8}
        >
          <label for="searchButchery">
            Pesquise informando o CNPJ do açougue
          </label>
                ...
                {data.map((butchery) => (
                  <GridItem xs={12} sm={6} md={6}>
                    <Card>
                      <CardHeader
                        color={`${
                          butchery.operation === "Opened" ? "success" : "danger"
                        }`}
                        stats
                        icon
                      >
                        ...
                        >
                          <CardAvatar profile>
                            <a href="#" onClick={(e) => e.preventDefault()}>
                              <Img
                                src={butchery.logo ? butchery.logo.url : logo}
                                alt="..."
                              />
                            </a>
                          </CardAvatar>
                        ...
                        <h3 className={classes.cardTitle}>
                          ...
                          butchery.responsible.length > 25
                        ...
                        <GridContainer>
                          <GridItem xs={12} sm={6} md={6}>
                            <Button
                              type="button"
                              color="info"
                              onClick={() => handleForm(butchery.id)}
                            >
                              Visualiar Dados
                            </Button>
                          </GridItem>
